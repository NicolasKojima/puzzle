...
#is quite optimized and has the time and the attempts 
...
if solve(board, all_shapes):
    ...
    ax.set_xticklabels([])
    ax.set_yticklabels([])
    plt.title("Final Board Configuration")
    plt.text(0, 8.5, f"Solved in {time_taken:.4f} seconds with {attempts} attempts", ha='left', fontsize=12)
    plt.show()
else:
