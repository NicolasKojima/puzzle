...
# Define the shapes and their possible rotations/mirrors
shapes = [
...
all_shapes = []
for shape in shapes:
    all_shapes.extend(generate_all_rotations_and_mirrors(shape))
...
def solve(board, shapes, index=0, ax=None):
    if index == len(shapes):
        return True
        ...
        for x in range(8):
            for shape in shapes[index]:
                if can_place(board, shape, x, y):
                    ...
                    plot_board(board, ax)
                    if solve(board, shapes, index + 1, ax):
                        return True
...
if solve(board, [generate_all_rotations_and_mirrors(shape) for shape in shapes], ax=ax):
